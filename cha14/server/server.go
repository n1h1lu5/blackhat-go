...
// mustEmbedUnimplementedAdminServer implements [grpcapi.AdminServer].
...
func (s *implantServer) FetchCommand(ctx context.Context, empty *grpcapi.Empty) (*grpcapi.Command, error) {
	...
	case cmd, ok := <-s.work:
		...
		return cmd, errors.New("work channel closed")
	default:
		// No work
		return cmd, nil
	}
...
func (s *implantServer) SendOutput(ctx context.Context, cmd *grpcapi.Command) (*grpcapi.Empty, error) {
	s.output <- cmd
